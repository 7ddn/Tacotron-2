...
def add_train_stats(model, hparams):
	with tf.compat.v1.variable_scope('stats') as scope:
		...
		if hparams.predict_linear:
			...
			for i in range(hparams.tacotron_num_gpus):
				tf.compat.v1.summary.histogram('linear_outputs %d' % i, model.tower_linear_outputs[i])
				tf.compat.v1.summary.histogram('linear_targets %d' % i, model.tower_linear_targets[i])
		...
		if hparams.tacotron_teacher_forcing_mode == 'scheduled':
			tf.compat.v1.summary.scalar('teacher_forcing_ratio', model.ratio) #Control teacher forcing ratio decay when mode = 'scheduled'
...
def add_eval_stats(summary_writer, step, linear_loss, before_loss, after_loss, stop_token_loss, loss):
	values = [
	tf.compat.v1.Summary.Value(tag='Tacotron_eval_model/eval_stats/eval_before_loss', simple_value=before_loss),
	tf.compat.v1.Summary.Value(tag='Tacotron_eval_model/eval_stats/eval_after_loss', simple_value=after_loss),
	tf.compat.v1.Summary.Value(tag='Tacotron_eval_model/eval_stats/stop_token_loss', simple_value=stop_token_loss),
	tf.compat.v1.Summary.Value(tag='Tacotron_eval_model/eval_stats/eval_loss', simple_value=loss),
	]
	if linear_loss is not None:
		values.append(tf.compat.v1.Summary.Value(tag='Tacotron_eval_model/eval_stats/eval_linear_loss', simple_value=linear_loss))
...
def train(log_dir, args, hparams):
	...
	if hparams.GL_on_GPU:
		GLGPU_mel_inputs = tf.compat.v1.placeholder(tf.float32, (None, hparams.num_mels), name='GLGPU_mel_inputs')
		GLGPU_lin_inputs = tf.compat.v1.placeholder(tf.float32, (None, hparams.num_freq), name='GLGPU_lin_inputs')
